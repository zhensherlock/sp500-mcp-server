...
export function registerGetCompanyFilingsTool(mcpServer: McpServer) {
  mcpServer.registerTool(
    ...
    async (params: z.infer<typeof GetCompanyFilingsParams>) => {
      ...
      const { data } = await filingsQuery.order('filing_date', { ascending: false }).limit(limit)
      ...
      if (!data?.length) {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `No filings found for ${symbol}${filterText}. Try adjusting the date range or filing type.`,
            },
          ],
        }
      }
