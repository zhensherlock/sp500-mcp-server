  ...
  mcpServer.registerTool(
    ...
    async (params: z.infer<typeof getCompanyNewsParams>) => {
      ...
      let newsQuery = supabase
        ...
        .select(
          'symbol, title, summary, provider, pubDate, thumbnail, url, lm_level, lm_score1, lm_score2, lm_sentiment',
        )
        .eq('symbol', symbol)
      ...
      if (!data?.length) {
        ...
        return {
          content: [{ type: 'text', text: `No news found for ${symbol}${sentimentFilter}.` }],
        }
      }
