...
- Next.js App Router MCP endpoint is `apps/web/app/[transport]/route.ts`; with `basePath: '/'`, tests and proxy call it at `/sse`.

## Commands

- Install: `pnpm install`
- Dev server: `pnpm dev`; focused Next app only: `pnpm --filter @apps/web dev`; focused MCP App UI only: `pnpm --filter @apps/web-app dev` (proxies `/sse` to `localhost:3000`).
...
- Vitest setup in `apps/web/vitest.setup.ts` connects a real MCP client to `http://localhost:3000/sse`; tests are integration tests and fail if the dev server or Supabase data is unavailable.
