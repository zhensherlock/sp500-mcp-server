...
import type { AppStatus, CompanyNewsResult } from './types'
import { parseCompanyNewsFromToolResult } from './utils'

export function useCompanyNewsApp() {
  const [result, setResult] = useState<CompanyNewsResult | null>(null)
  const [query, setQuery] = useState<string | null>(null)
  const [parseError, setParseError] = useState<string | null>(null)

  const handleToolResult = useCallback((toolResult: unknown) => {
    try {
      setResult(parseCompanyNewsFromToolResult(toolResult))
      setParseError(null)
    } catch (error) {
  ...
  const { isConnected, error } = useApp({
    appInfo: { name: 'SP500 Company News', version: '1.0.0' },
    capabilities: {},
    onAppCreated: app => {
