...
import type { AppStatus, CompanyFilingsResult } from './types'
import { parseCompanyFilingsFromToolResult } from './utils'

export function useCompanyFilingsApp() {
  const [result, setResult] = useState<CompanyFilingsResult | null>(null)
  const [query, setQuery] = useState<string | null>(null)
  const [parseError, setParseError] = useState<string | null>(null)

  const handleToolResult = useCallback((toolResult: unknown) => {
    try {
      setResult(parseCompanyFilingsFromToolResult(toolResult))
      setParseError(null)
    } catch (error) {
  ...
  const { isConnected, error } = useApp({
    appInfo: { name: 'SP500 Company Filings', version: '1.0.0' },
    capabilities: {},
    onAppCreated: app => {
