  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent
        ...
        style={{ width: "90vw", maxWidth: 896, maxHeight: "90vh" }}
      >
        <DialogHeader>
          <DialogTitle>Test {toolName}</DialogTitle>
          <DialogDescription>
            Enter parameters to execute this tool
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-4 py-4">
          {params.map((param) => (
            <div key={param.name} className="flex flex-col gap-1.5">
              <label
                htmlFor={`dialog-param-${toolName}-${param.name}`}
          ...
          <div className="relative p-4">
            {error && (
              <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-4">
                <pre className="font-mono text-sm text-destructive whitespace-pre-wrap break-all">
                  {error}
                </pre>
              </div>
            )}
